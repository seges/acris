...
public class FieldSpecLoaderCreator implements ILoaderCreator {
	...
	public boolean generateLoader(SourceWriter sourceWriter,
			...
			String targetListWidgetName, String beanProxyWrapper, String binding)
		...
		sourceWriter.println(dataLoaderClass.getCanonicalName() + " " + lowerClassName + "DataLoader = new " + dataLoaderClass.getCanonicalName() + "();");
		
		sourceWriter.println(lowerClassName + "DataLoader.load(" + Page.class.getSimpleName() + ".ALL_RESULTS_PAGE, new " + ICallback.class.getSimpleName() + "<" + PagedResult.class.getSimpleName() + "<" + List.class.getSimpleName() + "<" + classType.getSimpleSourceName() + ">>>() {");
		sourceWriter.indent();
