package sk.seges.acris.security.server.user_management.dao.appengine;
...
public abstract class AbstractTwigCrud<T extends IMutableDomainObject<Long>> implements ICrudDAO<T> {
	...
	protected AbstractTwigCrud(ObjectDatastore datastore, Class<T> clazz) {
	...
	public PagedResult<List<T>> findAll(Page requestedPage) {
		RootFindCommand<T> findCommand = datastore.find().type(clazz);
		...
		Iterator<T> result = findCommand.returnResultsNow();
	...
	private RootFindCommand<T> applyProjections(RootFindCommand<T> findCommand, Page page) {
		//TODO
		return findCommand;
	}
	
	private RootFindCommand<T> applyFilters(RootFindCommand<T> findCommand, Page page) {
	...
	private RootFindCommand<T> applySortCriteria(RootFindCommand<T> findCommand, Page page) {
	...
	private RootFindCommand<T> applyPageSize(RootFindCommand<T> findCommand, Page page) {
