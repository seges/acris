 ...
 */
public class CacheableServerSerializationStreamWriter extends AbstractSerializationStreamWriter {
    ...
    private enum ValueWriter {
        BOOLEAN {
            @Override
            void write(CacheableServerSerializationStreamWriter stream, Object instance) {
        ...
        abstract void write(CacheableServerSerializationStreamWriter stream, Object instance)
                throws SerializationException;
    }
    ...
    private int idsListCharCount;

    public CacheableServerSerializationStreamWriter(SerializationPolicy serializationPolicy) {
    ...
    public String toString() {
        ...
        //
        int capacityGuess = REMOVE_PREFIX.length() * 4 + 2 * idsListCharCount + 2 * tokenListCharCount + 2 * tokenList.size();
        ServerSerializationStreamWriter.LengthConstrainedArray stream = new ServerSerializationStreamWriter.LengthConstrainedArray(capacityGuess);
        writeIds(stream);
        writePayload(stream);
    ...
    private void serializeClass(Object instance, Class<?> instanceClass)
        ...
        try {
            Method getIdMethod = instanceClass.getMethod("getId");

            if (getIdMethod != null) {
                getIdMethod.setAccessible(true);
                Object idValue = getIdMethod.invoke(instance);

                if (idValue != null) {
                    writeId(instanceClass.getCanonicalName() + "/" + idValue.hashCode());
                }
            }
            ...
            //it has the method, but it's not accessible, but we tried to
            //change the accessibility using setAccessible method, so this could not happend ... or?
            e.printStackTrace();
        }
     ...
     */
    private void writeHeader(ServerSerializationStreamWriter.LengthConstrainedArray stream) {
    ...
    private void writeIds(ServerSerializationStreamWriter.LengthConstrainedArray stream) {
        ...
        while (idIterator.hasNext()) {
            stream.addToken(idIterator.next());
        }
        stream.addToken(REMOVE_PREFIX);
    }

    private void writePayload(ServerSerializationStreamWriter.LengthConstrainedArray stream) {
    ...
    private void writeStringTable(ServerSerializationStreamWriter.LengthConstrainedArray stream) {
        ServerSerializationStreamWriter.LengthConstrainedArray tableStream = new ServerSerializationStreamWriter.LengthConstrainedArray();
        for (String s : getStringTable()) {
            tableStream.addToken(ServerSerializationStreamWriter.escapeStringSplitNodes(s));
        }
