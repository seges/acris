...
//		getPager().setPageStart(range.getStart());
	...
	public Criterion getDefaultCriteria() { return null; }
	...
	public static class RenderableDateCell<T> extends DateCell {
		...
		public RenderableDateCell(AbstractFilterableTable<T> cellTable, CellFormatter<Date, T> cellFormatter, CellRenderer<Date, T> cellRenderer) {
		...
		public void render(Context context, Date value, SafeHtmlBuilder sb) {
			int index = (context.getIndex() - ((cellTable.getPageStart() / cellTable.getPageSize()) * cellTable.getPageSize()));
			T visibleItem = cellTable.getVisibleItem(index);

			if (cellRenderer != null) {
				com.google.gwt.dom.client.Style style = cellTable.getRowElement(index).getCells().getItem(context.getColumn()).getStyle();
//				cellRenderer.renderCell(style, value, visibleItem);
	...
	public void addDateColumn(final Column<T, ?> column, int width, String text, String property, Validator<Date> dateValidator) {
		...
		FilterableDateHeader filterableDateHeader = new FilterableDateHeader(dateUpdater, dateFilter.getCriterion(property, defaultLoVal, defaultHiVal), dateValidator, text);
	...
	public <F extends Comparable<? extends Serializable>> void addTextColumn(final Column<T, ?> column, int width, String text, String property, Validator<F> validator) {
		addTextColumn(column, width, text, property, validator, Filter.EQ);
	}
	
	@SuppressWarnings("unchecked")
	public <F extends Comparable<? extends Serializable>> void addTextColumn(final Column<T, ?> column, int width, String text, String property, Validator<F> validator, String operator) {
		...
		this.addColumn(column, new FilterableTextHeader<F>(columnUpdater, textFilter.getCriterion(property, defaultVal), validator, text));
		this.setColumnWidth(column, width, Unit.PCT);
		column.setSortable(true);
	}

	public <F extends Enum<?>> void addEnumeratedColumn(final Column<T, ?> column, int width, String text, String property, List<String> options, Validator<F> validator) {
		addEnumeratedColumn(column, width, text, property, options, validator, Filter.EQ);
	}
	
	@SuppressWarnings("unchecked")
	public <F extends Enum<?>> void addEnumeratedColumn(final Column<T, ?> column, int width, String text, String property, List<String> options, Validator<F> validator, String operation) {
		...
		this.addColumn(column, new FilterableSelectionHeader<F>(columnUpdater, textFilter.getCriterion(property, defaultVal), validator, options, text));
		this.setColumnWidth(column, width, Unit.PCT);
		column.setSortable(true);
	}
	...
	private <F extends Comparable<? extends Serializable>> void handleFilterValueChange(InputFilter<F> dataTypeFilter, SimpleExpression<F> value, Column<T, ?> column) {
	...
	public Page getPage() {
		...
		if (defaultCriteria != null && conjunction != null) {
			conjunction.add(defaultCriteria);
			page.setFilterable(conjunction);
		} else if (defaultCriteria != null) {
