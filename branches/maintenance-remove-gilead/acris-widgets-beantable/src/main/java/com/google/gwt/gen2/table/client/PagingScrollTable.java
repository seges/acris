...
public class PagingScrollTable<RowType> extends AbstractScrollTable implements HasTableDefinition<RowType>,
		HasPageCountChangeHandlers, HasPageLoadHandlers, HasPageChangeHandlers, HasPagingFailureHandlers {
	/**
	...
	protected static class PagingScrollTableCellView<RowType> extends AbstractCellView<RowType> {
		...
		public void setStyleAttribute(String attr, String value) {
			table.getDataTable().getFixedWidthGridCellFormatter().getRawElement(getRowIndex(), getCellIndex())
					.getStyle().setProperty(attr, value);
		}
	...
	protected static class PagingScrollTableRowView<RowType> extends AbstractRowView<RowType> {
		...
		@Override
		public void setStyleAttribute(String attr, String value) {
			table.getDataTable().getFixedWidthGridRowFormatter().getRawElement(getRowIndex()).getStyle()
	...
	public PagingScrollTable(TableModel<RowType> tableModel, TableDefinition<RowType> tableDefinition) {
		this(tableModel, new CustomFixedWidthGrid(), new FixedWidthFlexTable(), tableDefinition);
	...
	public PagingScrollTable(TableModel<RowType> tableModel, TableDefinition<RowType> tableDefinition,
		...
		this(tableModel, new CustomFixedWidthGrid(), new FixedWidthFlexTable(), tableDefinition, resources);
	}
	 ...
	 */
	public PagingScrollTable(TableModel<RowType> tableModel, CustomFixedWidthGrid dataTable,
			FixedWidthFlexTable headerTable, TableDefinition<RowType> tableDefinition, ScrollTableResources resources) {
		...
		if (tableModel instanceof HasRowValueChangeHandlers) {
			((HasRowValueChangeHandlers<RowType>) tableModel)
					.addRowValueChangeHandler(new RowValueChangeHandler<RowType>() {
						@Override
		...
		if (dataTable.getColumnFilter() == null) {
			ColumnFilterer filterer = new ColumnFilterer() {
				...
				public void onFilterColumn(SortableGrid grid, ColumnFilterList filterList,
						final ColumnFilterCallback callback) {
					if (timer != null) {
						timer.cancel();
					}
					timer = new Timer() {
	...
	/**
	 * Get the selected row values. If cross page selection is enabled, this
	 * will include row values selected on all pages.
	 * 
	...
	public Set<RowType> getSelectedRowValues() {
		...
		Set<RowType> selectedRowValues = new HashSet<RowType>();
	...
	public void gotoPage(int page, boolean forced) {
		...
		if (currentPage != oldPage || forced) {
			...
			lastRequest = createRequest(firstRow, pageSize, dataTable.getColumnSortList(),
	...
	/**
	 * Enable or disable cross page selection. When enabled, row value
	 * selections are maintained across page loads. Selections are remembered by
	 * type (not by row index), so row values can move around and still maintain
	 * their selection.
	 * 
	 * @param enabled
	 ...
	 * 
	 */
	protected Request createRequest(int startRow, int pageSize, ColumnSortList columnSortList,
	...
	protected void populateHeaderTable(FixedWidthFlexTable headerTable, TableDefinition tableDefinition) {
		int column = 0;
		for (ColumnDefinition<RowType, ?> columnDefinition : ((TableDefinition<RowType>) tableDefinition)
				.getVisibleColumnDefinitions()) {
			Object header = columnDefinition.getColumnProperty(HeaderProperty.TYPE).getHeader(0);
			if (header != null) {
		...
		if (filterRow > 0) {
			column = 0;
			for (ColumnDefinition<RowType, ?> colDef : ((TableDefinition<RowType>) tableDefinition)
					.getVisibleColumnDefinitions()) {
	 ...
	 * 
	 * @param table
	 *            the header or footer table
	 * @param allInfos
	 *            the header info
	 * @param isHeader
	 *            false if refreshing the footer table
	 */
