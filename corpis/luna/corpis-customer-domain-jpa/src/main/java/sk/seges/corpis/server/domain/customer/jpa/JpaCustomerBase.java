...
import sk.seges.corpis.shared.domain.customer.base.AddressBase;
import sk.seges.corpis.shared.domain.customer.base.CustomerBase;
import sk.seges.corpis.shared.domain.customer.validation.CompanyCustomerFormCheck;
...
@AssociationOverride(name = CustomerBase.CORRESP_ADDRESS + "." + AddressBase.COUNTRY, joinColumns = @JoinColumn(name = DBConstraints.CORRESP_TABLE_PREFIX
			+ AddressBase.COUNTRY + "_id"))
public class JpaCustomerBase extends CustomerBase<Integer> {
	...
	public boolean isCompanyCustomerType() {
		return (company != null && company.getCompanyName() != null && company.getCompanyName().length() > 0);
	}
}
