...
public class CopyFromDtoMethodPrinter extends AbstractMethodPrinter implements CopyMethodPrinter {
	...
	public void printCopyMethod(ProcessorContext context, FormattedPrintWriter pw) {
		...
		String previousPath = ElementPrinter.RESULT_NAME;
		...
		if (nested && context.getConfigurationTypeElement().getDomain() != null) {
			...
			while (pathResolver.hasNext()) {
				...
				if (!instances.contains(fullPath)) {
					...
					if (referenceDomainType.asType().getKind().equals(TypeKind.DECLARED)) {
						...
						if (referenceDomainType.getId(entityResolver) != null) {
							...
							pw.println(".getDomainInstance(" + ElementPrinter.DTO_NAME + "." + MethodHelper.toGetter(fullPath + MethodHelper.toMethod(MethodHelper.toField(referenceDomainType.getIdMethod(entityResolver)))) + ");");
							instances.add(fullPath);
						} else {
							pw.println(referenceDomainType + " " + currentPath + " = " + ElementPrinter.RESULT_NAME + "." + MethodHelper.toGetter(currentPath) + ";");
							pw.println("if (" + ElementPrinter.RESULT_NAME + "." + MethodHelper.toGetter(currentPath) + " == null) {");
			...
			if (context.getConfigurationTypeElement().getDomain() != null && domainTypeElement.getSetterMethod(context.getDomainFieldPath()) != null) {
				if (context.getConverter() != null) {
					...
					pw.println(";");
					pw.print(ElementPrinter.RESULT_NAME + "." + MethodHelper.toSetter(context.getDomainFieldPath()) + "("+ converterName + ".fromDto(");
					pw.print("(" + castToDelegate(context.getDomainMethodReturnType()).toString(ClassSerializer.CANONICAL, true) + ")");
					pw.print(ElementPrinter.DTO_NAME  + "." + MethodHelper.toGetter(context.getFieldName()));
				} else {
					pw.print(ElementPrinter.RESULT_NAME + "." + MethodHelper.toSetter(context.getDomainFieldPath()) + "(" + ElementPrinter.DTO_NAME + "." + MethodHelper.toGetter(context.getFieldName()));
				}
				if (context.getConverter() != null) {
					pw.print(")");
				}
			...
			if (context.getConverter() != null) {
				...
				converterProviderPrinter.printDomainConverterMethodName(context.getConverter(), context.getDomainMethodReturnType(), pw);
				pw.println(";");
				pw.print(ElementPrinter.RESULT_NAME + "." + MethodHelper.toSetter(context.getDomainFieldPath()) + "(");
				pw.print("(" + castToDelegate(context.getDomainMethodReturnType()).toString(ClassSerializer.CANONICAL, true) + ")");
				pw.print(converterName + ".fromDto(" + ElementPrinter.DTO_NAME  + "." + MethodHelper.toGetter(context.getFieldName()));
			} else if (context.getLocalConverterName() != null) {
				pw.println("if (" + context.getLocalConverterName() + " != null) {");
				pw.print(ElementPrinter.RESULT_NAME + "." + MethodHelper.toSetter(context.getDomainFieldPath()) + "(" + 
						context.getLocalConverterName() + ".fromDto(" + ElementPrinter.DTO_NAME  + "." + MethodHelper.toGetter(context.getFieldName()) + ")");
			} else {
