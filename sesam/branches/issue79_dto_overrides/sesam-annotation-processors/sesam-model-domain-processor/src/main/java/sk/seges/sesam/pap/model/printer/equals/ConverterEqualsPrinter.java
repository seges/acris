...
public class ConverterEqualsPrinter extends AbstractDtoPrinter implements TransferObjectElementPrinter {
	...
	public ConverterEqualsPrinter(ConverterProviderPrinter converterProviderPrinter, EntityResolver entityResolver, TransferObjectProcessingEnvironment processingEnv, FormattedPrintWriter pw) {
		super(processingEnv);
	...
	public void initialize(ConfigurationTypeElement configurationTypeElement, MutableDeclaredType outputName) {
		...
		if (entityResolver.shouldHaveIdMethod(configurationTypeElement.getDomain())) {
			...
			if (domainId.getConverter() != null) {
				...
				converterProviderPrinter.printDomainConverterMethodName(domainId.getConverter(), domainId, methodName, configurationTypeElement.getDomain().getIdMethod(entityResolver), pw);
				pw.print(".toDto(");
			} else {
