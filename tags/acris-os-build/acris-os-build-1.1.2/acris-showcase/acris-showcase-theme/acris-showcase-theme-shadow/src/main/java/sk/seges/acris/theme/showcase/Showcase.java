package sk.seges.acris.theme.showcase;

import sk.seges.acris.theme.client.shadow.ShadowButtonPanel;
import sk.seges.acris.theme.client.shadow.ShadowCheckBoxPanel;
import sk.seges.acris.theme.client.shadow.ShadowComboBoxPanel;
import sk.seges.acris.theme.client.shadow.ShadowTextAreaPanel;
import sk.seges.acris.theme.client.shadow.ShadowTextBoxPanel;
import sk.seges.acris.widget.client.form.select.ComboBox;
import sk.seges.acris.widget.client.form.select.ComboBox.ComboBoxDataModel;

import com.google.gwt.core.client.EntryPoint;
import com.google.gwt.user.client.ui.Button;
import com.google.gwt.user.client.ui.CheckBox;
import com.google.gwt.user.client.ui.Grid;
import com.google.gwt.user.client.ui.Label;
import com.google.gwt.user.client.ui.RootPanel;
import com.google.gwt.user.client.ui.TextArea;
import com.google.gwt.user.client.ui.TextBox;


public class Showcase implements EntryPoint {

	@Override
	public void onModuleLoad() {

		Grid grid = new Grid(8, 2);
		RootPanel.get().add(grid);
		
		int i = 0;
		grid.setWidget(i/2, i%2, new Label("Name:"));
		i++;
		grid.setWidget(i/2, i%2, (TextBox)new ShadowTextBoxPanel());
		i++;

		grid.setWidget(i/2, i%2, new Label("E-mail:"));
		i++;
		grid.setWidget(i/2, i%2, (TextBox)new ShadowTextBoxPanel());
		i++;

		grid.setWidget(i/2, i%2, new Label("Telephone:"));
		i++;
		grid.setWidget(i/2, i%2, (TextBox)new ShadowTextBoxPanel());
		i+=2;

		CheckBox cb = (CheckBox)new ShadowCheckBoxPanel();
		cb.setText("Important");
		grid.setWidget(i/2, i%2, cb);
		i+=2;

		cb = (CheckBox)new ShadowCheckBoxPanel();
		cb.setText("Request response");
		cb.setEnabled(false);
		cb.setValue(true);
		grid.setWidget(i/2, i%2, cb);
		i+=2;

		TextArea ta = (TextArea)new ShadowTextAreaPanel();
		grid.setWidget(i/2, i%2, ta);
		i+=2;

		Button button = (Button)new ShadowButtonPanel();
		button.setText("Send");
		grid.setWidget(i/2, i%2, button);
		i+=2;
		
		ComboBox combo = new ShadowComboBoxPanel();
		
		ComboBoxDataModel model = new ComboBoxDataModel();
		combo.setModel(model);
		combo.display();
		
		combo.addItems("value1", "value2", "value3");
		grid.setWidget(i/2, i%2, combo);
		
	}
}